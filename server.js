...
const sequelize = new Sequelize(
  "postgres://postgres:password@localhost:5432/tag"
...
async function oldSocketID(socketID, gameID) {
  return await Player.findOne({
    where: {
      [Sequelize.Op.and]: [{ GameId: gameID }, { socket_id: socketID }],
    },
  });
}
...
async function findHostOfGame(gameInst) {
  var playerInst = await Player.findOne({
    where: {
      [Sequelize.Op.and]: [{ GameId: gameInst.id }, { host: true }],
    },
  });
...
io.on("connection", (socket) => {
  ...
  async function updateLobbyScreens(playersList, gameInst) {
    ...
    io.to(gameInst.id).emit("validGame", {
      playersList: playersList,
      status: gameInst.status,
      host: await findHostOfGame(gameInst),
    });
  }
  ...
  async function joinGame(gameId, socketId, nickname, host = false) {
    ...
    players.forEach((player) =>
      playersList.push([player.type, player.nickname, player.score])
    );
  ...
  socket.on("attemptToJoinGameFromCookie", function (data) {
    (async () => {
      ...
      if (data.gameCode) {
        var gameInst = await Game.findOne({
          where: {
            [Sequelize.Op.and]: [{ url_id: data.gameCode }, { status: "I" }],
          },
        });
        if (gameInst) {
          socket.emit("joinGameOnClient", {
            gameCode: gameInst.url_id,
            clientPlayerNickname: data.nickname,
          });
        }
      }
    })();
  ...
  socket.on("joinGame", function (data) {
    (async () => {
      ...
      var gameInst = await Game.findOne({
        where: {
          [Sequelize.Op.and]: [{ url_id: data.url_id }, { status: "W" }],
        },
      });
  ...
  socket.on("createGame", function (data) {
    (async () => {
      var gameInst = await createGame({
        round_nums: data.round_nums,
        public_game: data.public_game,
      });
  ...
  socket.on("getRandomCode", function () {
    (async () => {
      ...
      var gameInst = await Game.findOne({
        where: {
          [Sequelize.Op.and]: [{ public_game: true }, { status: "W" }],
        },
